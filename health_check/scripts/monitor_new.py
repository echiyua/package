...
# moshell = ["0.164.133.105", "10.164.133.104", "10.164.133.103", "10.164.133.93", "10.164.133.107",
#            "10.164.133.108", "10.164.133.80", "10.164.133.81", "10.164.133.82", "10.164.133.83",
#            "10.164.133.84"
#            ]
...
moshellfile = localDir + r"/../" + r"configure/" + r"moshell_ip_bak.txt"
...
def analysis(result, moshell):
    ...
    if int(pmRadioUeRepCqi256QamRank1Distr[12]) > 20:
        try:
            with open(file2, 'w') as f2:
                string1 = "Fail:moshell:%s has serious Have serious interference problems,pmRadioUeRepCqi256QamRank1Distr[12]:%s" % (
                    moshell, pmRadioUeRepCqi256QamRank1Distr[12])
                f2.write(string1)
        except Exception as error:
            print 'error in open file in %s' % (time.strftime("%Y-%m-%d_%H.%M.%S", time.localtime()))
        finally:
            f2.close()
    elif int(pmRadioUeRepCqi256QamRank1Distr[13]) or int(pmRadioUeRepCqi256QamRank1Distr[14]) or int(
            pmRadioUeRepCqi256QamRank1Distr[15]):
        try:
            with open(file2, 'a+') as f2:
                string2 = "Fail:moshell:%s has serious Have serious interference problems,pmRadioUeRepCqi256QamRank1Distr:%s" % (
                    moshell, result)
                f2.write(string2)
        except Exception as error:
            print 'error in open file in %s' % (time.strftime("%Y-%m-%d_%H.%M.%S", time.localtime()))
        ...
        with open(file1, "a+") as f1:
            if not status_list[0]:
                ...
                if not status_list[1] and status_list[2]:  # have no version but have para
                    str3 = "moshell:%s\t%s\t%s\n" % (moshell, result_list[0], result_list[1])
                    f1.write(str3)
                if not status_list[1] and not status_list[2]:  # have no version but have para
                    str4 = "moshell:%s\t%s\t%s\n" % (moshell,result_list[0],result_list[1])
                    f1.write(str4)
    except Exception as error:
        print 'error in open file in %s' % (time.strftime("%Y-%m-%d_%H.%M.%S", time.localtime()))
