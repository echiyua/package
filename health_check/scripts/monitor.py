...
file2 = r"/tmp/echiyua/log/" + Date + "/serious_result.txt"
    ...
    if int(pmRadioUeRepCqi256QamRank1Distr[12]) > 20:
        try:
            with open(file2, 'a+') as f2:
                string1 = "FATAL:moshell:%s has serious Have serious interference problems:%s\n" % (moshell, result)
                f2.write(string1)
        except Exception as error:
        ...
        try:
            with open(file2, 'a+') as f2:
                string2 = "FATAL:moshell:%s has serious Have serious interference problems:%s\n" % (moshell, result)
                f2.write(string2)
        except Exception as error:
            print 'error in open file in %s' % (time.strftime("%Y-%m-%d_%H.%M.%S", time.localtime()))
            print error
